...
async function sendPhone(phone) {
  const result = await fetch(
    `${host}/sms/?action=send_code&type=update&phone=${phone}`
  );
...
class UpdatePhone extends React.Component {
  ...
  next = async () => {
    ...
    if (this.isNext()) {
      ...
      form.append("token", this.props.globalStore.token);
      form.append("phone", this.state.phone);
      form.append("code", this.state.code);
      const response = await fetchJson(
        "https://dostavka1.com/v1/user/update/index.php",
        {
          method: "POST",
          body: form
        }
      ...
      if (response) {
        if (response.errors) {
          if (response.errors.code != 200) {
            Alert.alert(response.errors.title, response.errors.detail);
          }
        } else {
          ...
          this.props.auth({
            data: {
              token: this.props.globalStore.token,
        ...
        this.codeInput.props.error = response.errors.title;
        this.setState({});
      }
    } else {
