...
export default class RestaurantMenu extends React.Component {
		...
		return (
			<View style={[
				styles.row, {
					...
					marginHorizontal: ((viewportWidth >= 320 && viewportWidth < 375) ? 20 : (viewportWidth >= 375 && viewportWidth < 414) ? 23.7 : 26),
					paddingVertical: 12,
				}
			]}>
				<Text style={{ color: '#fff', fontSize: 16, fontFamily: 'stem-medium', letterSpacing: 0.8}}>{section.type}</Text>
				<Icon
			...
			<View style={{flexDirection: 'column', width: viewportWidth}}>
				{section.plates.map((e, i) => {
					return <TouchableOpacity key={i} onPress={() => {
						...
						/>
						<View style={{ flexDirection: 'column', marginLeft: 10, width: viewportWidth - 20 - 20 - imageHeight }}>
							<Text style={{ color: '#fff', fontSize: 15, fontFamily: 'stem-medium', top: 3, lineHeight: 18, letterSpacing: 1 }}>{e.title}</Text>
							<Text style={{
		...
		return <View style={{ alignSelf: 'stretch' }}>
			<View style={[
				styles.row,
				{
					...
					borderWidth: 1,
					height: (viewportWidth >= 320 && viewportWidth < 375) ? 54 : (viewportWidth >= 375 && viewportWidth < 414) ? 63 : 70,
					marginTop: 31,
					marginBottom: 10,
					marginHorizontal: 15,
					alignItems: 'center',
			...
			<TouchableOpacity
				...
				<Text style={{ color: '#ffffff', fontSize: 14, fontFamily: 'stem-medium', top: 2}}>{title}</Text>
	...
	render() {
		var restaurant = <View ><ScrollView ref='scroll' style={styles.container} contentContainerStyle={{
			...
			/>
			<LinearGradient colors={['rgba(41,43,55, 0)', 'rgba(41,43,55, 1)']} style={{
				height: 100,
				position: 'absolute',
			...
			{this.state.hideInfo ? null : (<View
				style={{ alignItems: 'flex-start', alignSelf: 'stretch', marginTop: 0, marginBottom: 49, marginHorizontal: 20 }}
			>
			...
			(<View><View style={[styles.row, { justifyContent: 'center' }]}>
				<Text style={{
					color: '#dcc49c',
					marginBottom: 20,
					letterSpacing: 0.4,
					fontFamily: 'open-sans-semibold',
					fontSize: 13 }}>{'О ресторане'}</Text>
			</View>
			<View style={{
				justifyContent: 'center',
				left: -30
				...
				{/* Минимальный счет */}
				{this.renderAboutItem('clock', 'Время работы', this.state.data.time)}
				{/* Минимальный счет */}
				{this.renderAboutItem('coins', 'Средний счет', this.state.data.averageBill.toString() + '₽')}
				{/* Минимальный счет */}
				{this.renderAboutItem('truck', 'Минимальный счет', this.state.data.minBill.toString() + '₽')}
			</View></View>)}
			
			<TouchableOpacity onPress={() => this.setState({hideInfo: !this.state.hideInfo})}>
				<Text style={{
					textAlign: 'center',
					color: '#dcc49c',
					opacity: 0.5,
					...
					fontFamily: 'open-sans-semibold',
					fontSize: 13
				}}>{(this.state.hideInfo ? 'Показать' : 'Скрыть') + ' информацию \nо ресторане'}</Text>
			</TouchableOpacity>

			{/* Кнопка Прейти к меню */}
			{this.renderButton('Перейти к меню', () => { this.refs['scroll'].scrollToEnd({ animated: true })})}
			...
			{/* Меню ресторана */}
			<View style={[styles.row, { justifyContent: 'flex-start', marginHorizontal: 0, marginVertical: 13 }]}>
				<Text style={{
					color: '#FFF',
					fontSize: 16,
					fontFamily: 'stem-medium',
					letterSpacing: 0.8,
					marginLeft: (viewportWidth >= 320 && viewportWidth < 375) ? 20 : (viewportWidth >= 375 && viewportWidth < 414) ? 23.7 : 26
				}}>{'Меню ресторана'}</Text>
			</View>
			<View style={{ alignSelf: 'stretch', marginHorizontal: 10, height: 1, backgroundColor: 'rgb(87, 88, 98)' }} />
			<Accordion
				touchableProps={{
					activeOpacity: 0.2
				}}
